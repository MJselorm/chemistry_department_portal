      ...
      >
        ...
        <div className={`flex items-center gap-3 border-b border-[var(--sidebar-border)] px-4 py-4 ${isCollapsed ? 'lg:justify-center' : ''}`}>
          ...
          <button
            type="button"
            onClick={onClose}
            className="ml-auto grid h-10 w-10 place-items-center rounded-lg text-[var(--muted-foreground)] hover:bg-[var(--sidebar-hover)] hover:text-[var(--sidebar-foreground)] lg:hidden"
        ...
        {/* Navigation Items */}
        <nav className="p-3 flex-1 overflow-y-auto lg:overflow-visible space-y-1">
          {navLinks.map((item) => {
        ...
        <div className="p-3 border-t border-[var(--sidebar-border)] space-y-1 bg-[var(--sidebar-muted)]">
          <NavLink
            ...
            className={({ isActive }) =>
              `group relative flex min-h-11 items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${
                isCollapsed ? 'lg:justify-center' : ''
              } ${
                isActive
          ...
          <button
            ...
            className={`group relative w-full flex min-h-11 items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold text-[var(--destructive)] hover:bg-[var(--destructive-soft)] transition-colors text-left ${
              isCollapsed ? 'lg:justify-center' : ''
            }`}
